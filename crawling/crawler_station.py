...
# 주소 앞부분·행정구역 명칭 → 2글자 시도 (긴 키 우선 매칭)
ADDRESS_REGION_ALIASES: tuple[tuple[str, str], ...] = (
...
# load_station_csv·수동 적재용 고정 파일명
CANONICAL_CSV_NAME = "hydrogen_stations.csv"
...
class StationCrawler:
    ...
    async def _async_download(self) -> tuple[bytes, str]:
        ...
        async with async_playwright() as p:
            ...
            # 최신본 고정 경로 (load_station_csv 등에서 사용)
            canonical_path = os.path.join(self.download_dir, CANONICAL_CSV_NAME)
            shutil.copy2(save_path, canonical_path)
            print(f"[크롤러] CSV 저장 완료: {save_path}")
            print(f"[크롤러] 최신본 복사: {canonical_path}")
            ...
            with open(canonical_path, "rb") as file:
                return file.read(), canonical_path
    ...
    def _normalize_region(self, raw: str) -> str | None:
        if not raw or raw == "nan":
            return None
        raw = str(raw).strip()
        for prefix, region in sorted(
            ADDRESS_REGION_ALIASES, key=lambda x: len(x[0]), reverse=True
        ):
        ...
        # 앞 2글자로 최후 시도한다.
        short = raw[:2]
        return REGION_MAP.get(short, short if short in REGION_MAP else None)
