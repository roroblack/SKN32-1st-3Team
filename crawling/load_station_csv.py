...
# ── 설정 ──────────────────────────────────────────────────────
# crawler_station 실행 시 station_downloads/hydrogen_stations.csv 로 저장됨
_CRAWLING_DIR = os.path.dirname(os.path.abspath(__file__))
CSV_PATH = os.path.join(_CRAWLING_DIR, "station_downloads", "hydrogen_stations.csv")
...
def main():
    if not os.path.exists(CSV_PATH):
        ...
        print("  - python -m crawling.crawler_station 으로 먼저 다운로드하거나")
        print("  - data.go.kr CSV를 station_downloads/hydrogen_stations.csv 로 저장하세요.")
        sys.exit(1)
